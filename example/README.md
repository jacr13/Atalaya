...
Each run also produces an `info.json` file with the launch command, Python version, and other metadata; when executed inside a Git repository it automatically appends the remote, branch, commit hash, and ready-to-run clone/checkout commands for reproducing the run.
